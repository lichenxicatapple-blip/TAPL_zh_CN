...
`errata.txt` 文件内标明其内容更新至 2024-12-31。当前批次保存了第 4 章
对应的 `arith`、第 7 章对应的 `untyped`，以及第 5 章示例所使用的
`fulluntyped`。原始压缩包与解压内容都保留在仓库中；解压内容只作为原书
OCaml 行为和示例的核对依据，不作为本项目 Rust 代码的构建输入。
