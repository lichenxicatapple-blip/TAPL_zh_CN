...
% Logic entries use the shared translated-italic component to match the
% original italic convention without switching CJK font families.
\begin{figure}[p]
  \centering
  \scriptsize
  ...
  \begin{tabularx}{\textwidth}{
    ...
    年代 & 进展 & 参考文献 \\
    \midrule
    1870 年代 & \tapltranslateditalic{形式逻辑的起源} & Frege（1879） \\
    1900 年代 & \tapltranslateditalic{数学的形式化} & Whitehead 和 Russell（1910） \\
    1930 年代 & \tapltranslateditalic{无类型 lambda 演算} & Church（1941） \\
    1940 年代 & \tapltranslateditalic{简单类型 lambda 演算}
      & Church（1940）；Curry 和 Feys（1958） \\
    \addlinespace
    1950 年代 & Fortran & Backus（1981） \\
      & Algol-60 & Naur 等（1963） \\
    \addlinespace
    1960 年代 & \tapltranslateditalic{Automath 项目} & de Bruijn（1980） \\
      & Simula & Birtwistle 等（1979） \\
      & \tapltranslateditalic{Curry--Howard 对应} & Howard（1980） \\
      & Algol-68 & Van Wijngaarden 等（1975） \\
    \addlinespace
    1970 年代 & Pascal & Wirth（1971） \\
      & \tapltranslateditalic{Martin-Löf 类型论} & Martin-Löf（1973，1982） \\
      & \tapltranslateditalic{System \SystemF{}、\SystemFomega{}} & Girard（1972） \\
      & 多态 lambda 演算 & Reynolds（1974） \\
      & CLU & Liskov 等（1981） \\
      & 多态类型推断 & Milner（1978）；Damas 和 Milner（1982） \\
      & ML & Gordon、Milner 和 Wadsworth（1979） \\
      & \tapltranslateditalic{交类型} & Coppo 和 Dezani（1978） \\
    ...
    1980 年代 & NuPRL 项目 & Constable 等（1986） \\
      ...
      & \tapltranslateditalic{构造演算} & Coquand（1985）；Coquand 和 Huet（1988） \\
      & \tapltranslateditalic{线性逻辑} & Girard（1987）；Girard 等（1989） \\
      & 有界量化 & Cardelli 和 Wegner（1985） \\
      & & Curien 和 Ghelli（1992）；Cardelli 等（1994） \\
      & \tapltranslateditalic{爱丁堡逻辑框架} & Harper、Honsell 和 Plotkin（1992） \\
      & Forsythe & Reynolds（1988） \\
      & \tapltranslateditalic{纯类型系统}
