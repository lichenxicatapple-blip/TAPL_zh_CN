...
下面验证这些定义确实计算出并与交。论证分成两部分：命题 A.11 说明求得的交
是 \(S,T\) 的下界、求得的并是它们的上界；命题 A.12 说明求得的交大于每个
共同下界，求得的并小于每个共同上界。
...
引理 A.10 的论证可知不可能发生。若二者都是 \(\tapltype{Bool}\)，结论同样
立即成立。
...
\end{array}
\]
条件是两个元素类型在子类型关系下彼此等价；它们不必在句法上完全相同。
...
分支后，再加入以下四种组合，仍然是完备的。前面的引用等价分支优先于这里的
第一种组合：
\[
\begin{array}{rcl}
\tapltype{Ref}\ S_1\sqcup\tapltype{Ref}\ T_1
...
\[
\begin{array}{rcl}
\tapltype{Source}\ S_1\sqcap\tapltype{Source}\ T_1
 &=&\tapltype{Source}\ M_1
 \quad\text{若 }S_1\sqcap T_1=M_1\\
\tapltype{Ref}\ S_1\sqcap\tapltype{Source}\ T_1
 &=&\tapltype{Ref}\ S_1
 \quad\text{若 }S_1\subtype T_1\\
\tapltype{Source}\ S_1\sqcap\tapltype{Ref}\ T_1
 &=&\tapltype{Ref}\ T_1
 \quad\text{若 }T_1\subtype S_1
\end{array}
\]
两个 \(\tapltype{Ref}\) 的组合仍由上面的双向子类型分支处理；条件不满足时，
求交失败。
