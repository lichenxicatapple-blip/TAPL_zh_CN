...
\(P(i)\Rightarrow P(i+1)\)。由 \(F\) 的定义，\(X\subseteq P\) 蕴含
\(F(X)\subseteq P\)，所以 \(P\) 是 \(F\)-闭的。归纳原理给出
...
\(\mu\mathcal S\)。令
\[
R=\{(T(\pi),T(\pi))\mid\pi\in\{1,2\}^{*}\}.
\]
直接检查 \(\mathcal S\) 的定义可得 \(R\subseteq\mathcal S(R)\)，余归纳给出
\(R\subseteq\nu\mathcal S\)，故 \((T,T)\in\nu\mathcal S\)。另一方面，
\(\mu\mathcal S\) 只包含有限类型对：令 \(R'\) 为全部有限类型对的集合，归纳
原理给出 \(\mu\mathcal S\subseteq R'\)。

不存在属于 \(\nu\mathcal S_f\) 却不属于 \(\mu\mathcal S_f\) 的有限类型对，
因为两个不动点相等。对任意 \(S,T\in\mathcal T_f\)，命题
\((S,T)\in\nu\mathcal S_f\Rightarrow(S,T)\in\mu\mathcal S_f\) 可对有限树
\(T\) 归纳证明，分别讨论 \(\tapltype{Top}\)、积和箭头，并使用
\(\mathcal S_f(\nu\mathcal S_f)=\nu\mathcal S_f\) 与
...
\begin{proof}[证明概要]
对 \((n,k)\) 作字典序归纳，其中
\(k=\mu\text{-}\operatorname{height}(S)\)，
\(n=\mu\text{-}\operatorname{height}(T)\)。归纳把
