...
\inferrule*[right=\textsc{T-App}]
 {\vdash f:R_x\to\tapltype{Nat}
  \\
  \inferrule*[right=\textsc{T-Sub}]
   {\inferrule*[right=\textsc{T-Rcd}]{\cdots}{\vdash xy:R_{xy}}
    \\
    \inferrule*[right=\textsc{S-Trans}]
     {\inferrule*[right=\textsc{S-RcdWidth}]{ }{R_{xy}\subtype R_x}
      \\
      \inferrule*[right=\textsc{S-Refl}]{ }{R_x\subtype R_x}}
     {R_{xy}\subtype R_x}}
   {\vdash xy:R_x}}
 {\vdash f\,xy:\tapltype{Nat}}
\]
第二种做法还能任意插入更多自反与传递步骤。因此，本演算中每个可导出的类型
判断实际上都有无穷多棵不同的推导树。
...
反过来，若从子类型关系中删去某些判断 \(S\subtype T\)，使更少的类型构成
子类型关系，并不会破坏进展性或保持性。进展性定理只在前提中提到类型关系；
...
\inferrule*[right=\textsc{T-Sub}]
 {\inferrule*[right=\textsc{T-Sub}]
   {\Gamma\vdash t:S\\S\subtype U}{\Gamma\vdash t:U}
  \\
  U\subtype T}
 {\Gamma\vdash t:T}
\]
...
\item 对 \(S\subtype T_1\to T_2\) 的子类型推导作归纳。检查
  ...
  若为 \textsc{S-Refl}，则 \(S=T_1\to T_2\)，结论由自反性立即得到。
  若为 \textsc{S-Arrow}，其两个前提恰好就是所需结论。

  若为 \textsc{S-Trans}，则存在 \(U\)，使两个子推导分别以
  \(S\subtype U\) 和 \(U\subtype T_1\to T_2\) 为结论。对第二个子推导
  使用归纳假设，得到 \(U=U_1\to U_2\)、\(T_1\subtype U_1\) 和
  \(U_2\subtype T_2\)。现在已知 \(U\) 是箭头类型，可以对第一个子推导
...
\item 对 \(S\subtype\{l_i:T_i\}^{i\in1..n}\) 的子类型推导作归纳。
  ...
  \(U\subtype\{l_i:T_i\}^{i\in1..n}\)。对后一个子推导使用归纳假设，
  可知每个 \(l_i\) 都是某个 \(u_a\)，且对应字段满足
  \(U_a\subtype T_i\)。再对前一个子推导使用归纳假设，得到
