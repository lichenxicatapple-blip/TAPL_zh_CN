...
\begin{enumerate}
  ...
  \item 证明上述改动除了引入“内存耗尽”的可能性之外，不会改变最终结果。
    更具体地说，带垃圾收集的有限内存执行可以由不收集垃圾的无限存储执行
    模拟；反过来，不收集垃圾的无限存储执行也可以由有限内存、带垃圾收集的
    执行模拟，除非执行到下一步必须分配新位置而
    \(\mathit{reachable}(t,\mu)=L\)，此时内存耗尽。模拟所得的两个存储在
    共同定义域上取值相同；垃圾收集后的存储只保留仍可达的位置。
\end{enumerate}

\translatornote{作者官方勘误说明，原书本题第 5 项的 5a 与 5b 在初版中排写
混乱；上文依照勘误给出的意图，表述为“有限内存且带垃圾收集”与“无限存储且
不带垃圾收集”两种执行之间的比较。勘误没有给出唯一的替换公式，因而这里不
擅自补写比该说明更强的形式化结论。}
