...
\[
\begin{array}{ll}
\{\exists X,\{a:\tapltype{Nat},b:\tapltype{Nat}\}\},&
\{\exists X,\{a:X,b:\tapltype{Nat}\}\},\\
\{\exists X,\{a:\tapltype{Nat},b:X\}\},&
\{\exists X,\{a:X,b:X\}\}.
\end{array}
\]
加入子类型和有界量化后会增加许多，例如可以忘掉字段、把字段提升为
\(\tapltype{Top}\)，或使用上界 \(X<:\tapltype{Nat}\)。
...
一种办法是把重置计数器 ADT 嵌套在普通计数器 ADT 中。外包先隐藏
\(\tapltype{Counter}\)；其操作记录中再放一个存在包
\[
\begin{aligned}
\{\exists\ \tapltype{ResetCounter}<:\tapltype{Counter},
\{&\taplterm{new}:\tapltype{ResetCounter},\\
 &\taplterm{get}:\tapltype{ResetCounter}\to\tapltype{Nat},\\
 &\taplterm{inc}:\tapltype{ResetCounter}\to\tapltype{ResetCounter},\\
 &\taplterm{reset}:\tapltype{ResetCounter}\to\tapltype{ResetCounter}\}\}
\end{aligned}
\]
依次打开两层包后，上下文同时含
\(\tapltype{ResetCounter}<:\tapltype{Counter}\)，客户便可把重置计数器传给
普通计数器操作，而两种具体表示仍然隐藏。
