...
第（1）、（2）项需要在抽象和类型抽象情形中使用排列；第（3）、（4）项则是
在量词情形中使用排列。
      ...
      rcADT:
        {Some ResetCounter<:Counter,
         {new:ResetCounter,get:ResetCounter->Nat,
          ...
          reset:ResetCounter->ResetCounter}}}}
\end{lstlisting}
依次打开这两个包后，余下程序的上下文会含有类型变量绑定
\(\tapltype{Counter}<:\tapltype{Top}\) 与
\(\tapltype{ResetCounter}<:\tapltype{Counter}\)，以及项变量绑定
\(\taplterm{counter}:\{\ldots\}\) 与
\(\taplterm{resetCounter}:\{\ldots\}\)：
\begin{lstlisting}
...
只需在\cref{sec:24.3}的编码中显然的位置加入上界。在类型层面得到：
\[
\{\exists X<:S,T\}
\overset{\mathrm{def}}{=}
\forall Y.(\forall X<:S.T\to Y)\to Y
\]
项层面的改动直接由此得到。
