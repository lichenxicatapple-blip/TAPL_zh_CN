...
合一\cref{sol:translator:22.3.3}得到的约束：先由 \(A=B\to C\)，再代入
...
本书 Rust 对照实现位于
\path{code/book-snippets/src/chapter22.rs}，书内核心片段见
\taplnamedref{def:22.3.2}{定义22.3.2后的约束生成说明}和
\taplnamedref{def:22.4.4}{定义22.4.4后的合一说明}。测试覆盖恒等函数、把未知
函数用于自然数、自应用的出现检查失败，以及多步替换的传递应用。
...
其他右侧以不量化的单态方案加入上下文。至少应测试：同一恒等函数在一次 let
