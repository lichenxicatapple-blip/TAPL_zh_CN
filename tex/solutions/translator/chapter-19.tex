...
\taplrustsupportchunk{ch19-fj-class-table}{151}{186}
...
错误、继承环、未定义的类名、重复方法形参及保留名称 \texttt{this}。源码与测试
共同参与整书构建，因而书内片段与实际验证的实现保持一致。
...
为每个类生成 FJ 的标准构造函数，再擦除待求值项并交给上一题的 FJ 解释器。这样
书中的泛型实现不只检查擦除后的语法结构，还会实际执行擦除结果。
...
实参、递归上界、上界违反、方法体类型错误，以及擦除结果的实际求值。
...
\taplrustsupportchunk{ch19-gj}{601}{619}
