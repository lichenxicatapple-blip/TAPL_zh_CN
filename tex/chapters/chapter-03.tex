...
而是把守卫中的一步求值传递到整个条件式；直观上可以把它理解为一条
“上下文传播规则”。}
...
\translatornote{可以暂时把关系 \(R\) 想成一张“允许的一步求值表”：若
\((t,t')\in R\)，这张表就允许项 \(t\) 一步求值为 \(t'\)。规则
...
\cref{fig:3-1}三条规则能够推出的全部一步求值，并且不收录其他项对。
...
最小关系，正是为了排除这些多余项对，只留下规则所要求的一步求值。}
...
数值的形状，使用 \textsc{E-PredZero} 或 \textsc{E-PredSucc} 得出结果。
\par\smallskip
