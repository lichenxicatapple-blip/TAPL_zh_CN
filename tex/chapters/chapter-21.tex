...
\begin{definition}
\label{def:21.2.1}
\termfirst{树类型}{tree type}（简称树）是满足以下条件的偏函数
\[
...
\]
形式上，\(\mathcal T_f\) 是该文法描述的生成函数的最小不动点。若把同一
生成函数取最大不动点，就得到全部树类型 \(\mathcal T\)。
...
\end{cases}
\]
它从 \(X\) 开始不断加入支持集，直到得到一致集合，或发现不受支持的元素。
...
\begin{enumerate}
  ...
  \item 有些无限树也是正则的。例如
        ...
        \(\tapltype{Top}\) 两棵不同子树。
  \item 若无限积中相邻的 \(B\) 之间依次夹着越来越多个 \(A\)，所得树不正则。
        因而，证明 \(T\subtype T\) 所需的可达子类型对也会有无限多个。
\end{enumerate}
\end{example}
...
\subseteq\operatorname{subtrees}(S)\times\operatorname{subtrees}(T).
\]
右侧有限，故左侧也有限。
\end{proof}
...
\((|\pi|,\mu\text{-}\operatorname{height}(T))\)：箭头和积分支减小路径长度，
...
\(\nu\mathcal S_\mu\)，映射到树后属于 \(Q\)，所以积分支给出
...
\(\operatorname{TD}\) 的积分支得到 \(S\sqsubseteq T_1\times T_2\)。
其他分支相同。
\end{proof}
...
\(S_n\subtype T_n\) 时，箭头逆变会生成方向相反的子目标，几步之后同一层会
出现两个分别涉及 \(S_{n-1},T_{n-1}\) 的递归调用；每个调用又产生两个下一层
调用。故总调用次数与 \(2^n\) 成正比。
...
和 Park（1981）关于并发的工作；数学家更早便熟悉归纳的对偶形式，通用代数与
范畴论中都有明确发展。Aczel（1988）关于非良基集合的著作附有简短历史回顾。

Amadio 与 Cardelli（1993）给出了第一个递归类型子类型算法，并证明无限树的
包含关系、\(\mu\)-类型的算法关系和声明式参考关系三者等价。他们没有使用
余归纳，而用无限树的有限近似进行论证。Brandt 与 Henglein（1997）揭示了该
系统的余归纳本质，给出可靠且完备的归纳公理化，并详细证明子类型算法以
\(O(n^2)\) 时间终止；\taplsecref{21.9}{sec:21.9}紧随其证明。

Kozen、Palsberg 与 Schwartzbach（1993）把正则递归类型对应到带标签状态的
自动机，再通过积自动机和线性时间的空语言检查，得到优雅的二次时间算法。
Hosoya、Vouillon 与 Pierce（2001）把相关的树自动机方法用于面向 XML 的递归
类型与并类型。Jim 与 Palsberg（1999）讨论带子类型化和递归类型的类型重构，
同样把无限树子类型关系看作余归纳关系，并从给定类型对构造最小的一致集合。
