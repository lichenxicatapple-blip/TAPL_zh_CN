...
同一个变量可以同时出现在定义域和值域中。替换的各项同时生效。例如，替换
\[
...
unificand}，可以取代\cref{fig:22-1}约束生成规则中一条条的新鲜性条件。
Rémy（1992a, 1992b, 1998）的另一种改进是把赋型判断本身当作合一项：
从三元组 \((\Gamma,t,T)\) 出发，允许三个分量都含类型变量，再寻找使
\(\sigma\Gamma\vdash\sigma t:\sigma T\) 成立的替换 \(\sigma\)。}
...
直接成立；第 3 项由归纳假设和两个新变量集合不相交得到。对第 4 项，
新鲜性条件给出
...
let uvargen =
  ...
  in f 0
\end{taplocamlcode}
\taplrustcounterpartflowchunkfirst{ch22-types-generator}{1}{9}
\Needspace{30\baselineskip}
\taplrustcounterpartflowchunk{ch22-types-generator}{11}{37}
\Needspace{33\baselineskip}
\taplrustcounterpartflowchunk{ch22-types-generator}{39}{68}
\Needspace{21\baselineskip}
\taplrustcounterpartflowchunk{ch22-types-generator}{70}{87}
\Needspace{21\baselineskip}
\taplrustcounterpartflowchunk{ch22-types-generator}{88}{105}
...
\(C'\)。若 \(\sigma\) 是替换，则
\(\sigma C=\{\sigma S=\sigma T\mid S=T\in C\}\)，即对约束集逐元素作用
替换。满足约束集 \(C\) 的替换称为 \(C\) 的\termfirst{合一子}{unifier}：
\[
...
若语言允许递归类型，出现检查便可以去掉。\footnote{这个算法并不依赖
待合一的表达式恰好是类型；同一算法可以求解任意一阶表达式之间的等式约束。}
\end{definition}
...
let 右侧都会先得到含变量的类型，该变量在加入上下文前被泛化，以后每次实例化时又换成新变量。}
...
表达便利，却几乎不影响实际程序。Wright（1995）分析了大量以 1990 年版
