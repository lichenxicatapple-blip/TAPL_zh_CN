...
let uvargen =
  ...
  in f 0
\end{taplocamlcode}
\taplocamlgroupedcounterpart{ch22-types-generator}
这里的 \texttt{uvargen} 是一个函数；以 \texttt{()} 调用后，它返回
\texttt{NextUVar(x,f)}，其中 \texttt{x} 是新鲜类型变量名，\texttt{f} 又是同类
生成函数。

\taplrustcounterpartflowchunkfirst{ch22-types-generator}{1}{10}
\taplrustcounterpartflowchunk{ch22-inference-support}{2}{2}
\taplrustcounterpartflowchunk{ch22-types-generator}{41}{55}
...
下面的 Rust 对照实现采用先生成约束、再统一求解的版本，并把两步组合起来。
...
另一个问题是，若 \(x\) 在体中出现很多次，右侧就会被重复检查很多次，
不管它是否含有隐式标注的 lambda 抽象。由于右侧本身还可以包含 let 绑定，
这条规则会使类型检查器执行相对于原项大小呈指数增长的工作。
...
\end{enumerate}\footnote{引入泛化和实例化后，“显式标注了类型变量的 lambda 抽象”
...
使推断出的类型比表达式本身大得多。以下是 Jacques Garrigue 给出的更合适的
OCaml 例子：
\begin{taplocamlcode}
let l0 = ([], [], [], []) in
...
求值前就被复制，可以恢复安全性。此时 let 的求值规则改为\footnote{严格说来，
...
示例中的右侧是分配操作，不是值，所以把 \(r\) 加入上下文时，它获得的是
\(\tapltype{Ref}(X\to X)\)，而非类型方案
...
Cardelli（1987）的经典论文讨论了多项实现问题。Appel（1998）、Aho 等（1986）与
...
分别编译、“最聪明的重编译”、增量类型推断和精确定位类型错误；许多语言（尤其 ML）
...
\taplsecref{23.6}{sec:23.6}与\taplsecref{23.8}{sec:23.8}还会讨论这些结果，以及如何把 ML 风格重构
与秩 2 多态等更强的多态形式结合。
...
\termfirst{多态递归}{polymorphic recursion}。ML 中一条简单而没有问题的递归函数规则要求，
在函数自身定义的体中只能以单态方式使用该函数，即所有递归调用的参数和结果类型必须相同；
