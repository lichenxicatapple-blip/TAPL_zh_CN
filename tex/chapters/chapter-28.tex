...
\taplfigureheader{\(\to,\ \forall,\ <:\ \tapltype{Top}\)}
  {\(\lambda_{\to}\)（\taplnamedref{fig:16-3}{图}）}
...
证明；完全系统的相应修改见\taplnamedref{ex:28.2.3}{练习}。
...
\taplsecref{16.1}{sec:16.1}指出，带子类型简单类型 lambda 演算的声明式
子类型关系不是语法定向的，有两个原因。第一，S-Refl 和 S-Trans 的结论与
其他规则重叠；自下而上读取规则时，无法决定应试用哪一条。第二，S-Trans
的前提含有未出现在结论中的元变量，朴素算法必须猜测它。简单类型系统可以
删去这两条规则；不过在此之前，还要先把三条分立的记录子类型规则合并成
一条。

核 \(F_{<:}\) 的情况类似。有问题的仍是 S-Refl 和 S-Trans；删去它们之后，
还需略微调整余下的规则，以保留这两条被删规则的必要用法。
...
\inferrule*[right=S-Trans]
 ...
 {\Gamma\vdash Z<:W}
\]
幸好，这种形式的推导正是传递性的全部必要用法。新的 SA-Trans-TVar
把“查找变量紧接着使用传递性”这一模式合成一条规则；稍后将证明，用它替换
S-TVar 和 S-Trans，不会改变可导子类型判断的集合。
\[
\inferrule*[right=SA-Trans-TVar]
 {X<:U\in\Gamma\\\Gamma\algturnstile U<:T}
 {\Gamma\algturnstile X<:T}
\]
这些改动得到\taplnamedref{fig:28-3}{图}中的核 \(F_{<:}\) 算法子类型关系。
我们在转门上加一个箭头，以便在同时讨论两种关系时，区分算法子类型判断
和原始的声明式子类型判断。
...
\begin{minipage}{0.96\linewidth}
\taplfigureheader{\(\to,\ \forall,\ <:\ \tapltype{Top}\)}
  {\(\lambda_{\to}\)（\taplnamedref{fig:16-2}{图}）}
...
\operatorname{weight}_{\Gamma}(X)
 &=1+\operatorname{weight}_{\Gamma_1}(U)
   &&\text{若 }\Gamma=\Gamma_1,X<:U,\Gamma_2,\\
...
\operatorname{weight}_{\Gamma}(\forall X<:S.T)
 &=1+\operatorname{weight}_{\Gamma,X<:S}(T).
\end{aligned}
\]
...
+\operatorname{weight}_{\Gamma}(T).
\]
这项递归定义良好：每次递归调用时，上下文中所有类型的总大小，加上当前正在
定义权重的类型大小，总会严格减小。
\end{definition}
...
\centering
\begin{minipage}{0.96\linewidth}
\taplfigureheader{\(\to,\ \forall,\ <:\ \tapltype{Top}\ \textit{full}\)}
  {\taplnamedref{fig:28-3}{图}}
\taplfigureheaderplain{\(\Gamma\algturnstile S<:T\)}
  {\tapltranslateditalic{算法子类型关系}}
\[
\inferrule*[right=SA-Refl-TVar]{ }
...
引用声明式系统的收窄引理：它只保证构造出结论经收窄的新推导，不保证新推导与
原推导一样大。实际上，每当 S-TVar 查找被收窄的变量时，证明都要接入一份可能
任意大的推导，所以结果通常更大；这项拼接还会创建新的 S-Trans，而当前目的
正是证明没有 S-Trans 的算法关系仍具有传递性。因此，
...
\taplsolutionlink{sol:translator:28.5.2}{28.5.2}
...
\end{array}
\]
这里默默完成了加入新鲜类型变量时为保持上下文良构所需的重命名，并特意选择名称来
...
\end{fact}
\noindent\textit{证明练习（\(\star\star,\nrightarrow\)）。}
\taplsolutionlink{sol:translator:28.5.4}{28.5.4}
...
（Felty、Gunter、Hannan、Miller、Nadathur 与 Scedrov，1988）。
实际经验表明，下一节所述缺少并与交的问题（见\taplnamedref{ex:28.6.3}{练习}）比不可判定性更
棘手。

\begin{exercise}[\(\star\star\star\star\)]
\label{ex:28.5.6}
研究同时提供有界与无界量词、但没有 \(\tapltype{Top}\) 的系统；也就是说，
变量既可以有 \(X<:T\) 形式的绑定，也可以有无上界的 \(X\) 绑定。这个变体称为
\termfirst{完全有界量化}{completely bounded quantification}。
\begin{enumerate}
  \item 定义这个系统；
  \item 证明其子类型关系可判定；
  \item 这一限制是否真正解决了本节提出的根本问题？特别是，当语言还包含
        数字、记录、变体等构造时，结论是否仍然成立？
\end{enumerate}
\end{exercise}
...
交。完全 \(F_{<:}\) 不具备这两项性质（见\taplnamedref{ex:28.6.3}{练习}）。
...
\taplstatementlistbreak
\begin{enumerate}
  \item 若 \(\Gamma\vdash S<:V\) 且
        \(\Gamma\vdash T<:V\)，则存在 \(J\)，使
        \(\Gamma\vdash S\lor T=J\) 且
        \(\Gamma\vdash J<:V\)。
  \item 若 \(\Gamma\vdash L<:S\) 且
        \(\Gamma\vdash L<:T\)，则存在 \(M\)，使
...
第 2 项，再应用 SA-All。以上各情形先得到算法子类型判断，再由
\cref{thm:28.3.3}的可靠性得到命题所要求的声明式判断。
\end{proof}
