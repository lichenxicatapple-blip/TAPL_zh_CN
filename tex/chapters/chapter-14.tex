...
第~\ref{chap:13}~章说明了怎样用可变引用扩展纯简单类型 lambda 演算的
...
\S\ref{sec:11.10} 已经看到，有些异常条件可以通过让函数返回变体
（或 option）来表示。但若这些条件真正罕见，我们也许不愿强迫函数的每个
调用者都处理它们发生的可能。更合意的做法可能是：异常条件把控制直接转移到
程序更高层定义的异常处理器；甚至在异常足够罕见、或调用者无论如何也无法
恢复时，直接终止程序。本章先考察后一情形（\S\ref{sec:14.1}），把异常视为
整个程序的终止；再加入捕获异常并从中恢复的机制（\S\ref{sec:14.2}）；
最后细化两种机制，让程序员指定的额外数据可以从异常抛出点传给处理器
（\S\ref{sec:14.3}）。\footnote{本章研究在简单类型 lambda 演算
（图~\ref{fig:9-1}）上加入多种异常抛出与处理基本构造的系统
（图~\ref{fig:14-1} 与图~\ref{fig:14-2}）。第一种扩展的 OCaml 实现是
\texttt{fullerror}；携带值的异常语言（图~\ref{fig:14-3}）没有官方实现。}

\translatornote{本译本的 \texttt{code/simple/} Rust 参考实现同时覆盖
普通 \(\mathsf{error}\)、\(\mathsf{try}\) 处理器和携带值的
\(\mathsf{raise}\)，并验证传播次序与处理器行为。作者勘误指出，若干官方
OCaml 检查器的 \texttt{eval1} 漏掉了归约 \(\mathsf{try}\) 的分支；
Rust 版本采用图~\ref{fig:14-2} 的完整规则，不沿用这一实现缺漏。}
...
类似地，\textsc{E-AppErr2} 使用元变量 \(v_1\) 而不是遍历任意项的 \(t_1\)，
迫使求值器先把应用左端归约为值，再因为右端是 \(\mathsf{error}\) 而终止。
因此：
\[
(\mathsf{fix}\,(\lambda x:\tapltype{Nat}.\,x))\ \mathsf{error}
\]
会发散而不是终止。这些条件保证求值关系仍然确定。
...
语言中每个可赋型项具有唯一类型这一性质（定理~\ref{thm:9.3.3}）不再成立。
...
带异常语言的类型保持性质仍和以往一样：若项具有类型 \(T\)，让它求值一步，
结果仍具有类型 \(T\)。进展性质则要稍加细化。它原本说明良类型程序必须求值
...
\(\mathsf{error}\) 的求值规则可以理解为“展开调用栈”：不断丢弃尚未完成的
函数调用，直到错误一路传播到最外层。真实异常语言的实现正是这样工作：
调用栈由一组活动记录组成，每个活动函数调用对应一条；抛出异常会从调用栈中
依次弹出活动记录，直到栈空。
...
图~\ref{fig:14-2} 汇总的异常处理器形式与 ML、Java 都很相似。表达式
...
\S\ref{sec:14.1} 与 \S\ref{sec:14.2} 的机制让函数可以通知调用者“发生了
某种不寻常的事情”。通常还需要传回额外信息，说明具体发生了哪种事情，因为
处理器应该采取的行动——恢复后重试，或向用户呈现易懂的错误消息——可能
取决于这些信息。

图~\ref{fig:14-3} 说明怎样扩充基本异常处理构造，让每个异常携带一个值。
...
图~\ref{fig:14-1} 的 \textsc{E-AppErr1} 与 \textsc{E-AppErr2} 一样，
...
\S\ref{sec:14.2} 一样，说明主表达式归约为值后可以丢弃 try。
...
\begin{enumerate}
  ...
  \item 可以进一步把 \(T_{\mathit{exn}}\) 取作
    ...
    \(\mathsf{exception}\ l\ \mathsf{of}\ T\) 可理解成：“保证 \(l\)
    不同于 \(T_{\mathit{exn}}\) 中已有的全部标签；从现在开始，若声明前的
    可能变体为 \(l_1:T_1,\ldots,l_n:T_n\)，就令
    \[
      T_{\mathit{exn}}=
    ...
    (\lambda e:T_{\mathit{exn}}.\
      ...
      \mid y\Rightarrow\mathsf{raise}\ y).
    \end{aligned}
    \]
  ...
  \item Java 使用类而不是可扩展变体来支持用户定义异常。语言提供内建类
    ...
    的所有异常。二是 Java 区分\emph{异常}与\emph{错误}：前者是内建类
...
\label{ex:14.3.1}
上面第 4 种方案对可扩展变体类型的解释相当非正式。说明怎样把它精确化。
\end{exercise}

\begin{exercise}[\(\star\star\star\star\)]
\label{ex:14.3.2}
上文指出，Java 异常——\texttt{Exception} 的子类——比 ML 异常
（以及这里定义的异常）受到更严格控制：方法可能抛出的每个异常都必须在方法
类型中声明。扩展练习~\ref{ex:14.3.1} 的解答，让函数类型除了表示参数与
