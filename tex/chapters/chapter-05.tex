...
\begin{itemize}
  \item 在\termfirst{完全 \(\beta\) 归约}{full beta-reduction}下，任何
    ...
    三个可归约式。完全 \(\beta\) 归约可以先选最内层，再选中间，最后选
    最外层：
    \[
    \begin{aligned}
      \mathit{id}\,(\mathit{id}\,(\lambda z.\,\mathit{id}\,z))
...
\begin{aligned}
 ...
 &= (\lambda l.\,\lambda m.\,\lambda n.\,l\,m\,n)\ \mathit{tru}\ v\ w\\
 &\trans(\lambda m.\,\lambda n.\,\mathit{tru}\ m\ n)\ v\ w\\
 &\trans(\lambda n.\,\mathit{tru}\ v\ n)\ w\\
 &\trans\mathit{tru}\ v\ w\\
...
\begin{aligned}
 ...
 &=\mathit{fst}\,((\lambda f.\,\lambda s.\,\lambda b.\,b\,f\,s)\ v\ w)\\
 &\trans\mathit{fst}\,((\lambda s.\,\lambda b.\,b\,v\,s)\ w)\\
 &\trans\mathit{fst}\,(\lambda b.\,b\,v\,w)\\
 &=(\lambda p.\,p\,\mathit{tru})\,(\lambda b.\,b\,v\,w)\\
 &\trans(\lambda b.\,b\,v\,w)\,\mathit{tru}\\
 &\trans\mathit{tru}\ v\ w\\
 &\transmulti v.
\end{aligned}
\]
...
\begin{tikzpicture}[
  node distance=9mm,
  every node/.style={font=\small},
  pair/.style={draw,rounded corners=1pt,inner xsep=8pt,inner ysep=3pt},
  >=Stealth
]
  \node[pair] (p0) {\(\mathit{pair}\ c_0\ c_0\)};
  \node[pair,below=of p0] (p1) {\(\mathit{pair}\ c_0\ c_1\)};
  \node[pair,below=of p1] (p2) {\(\mathit{pair}\ c_1\ c_2\)};
  \node[pair,below=of p2] (p3) {\(\mathit{pair}\ c_2\ c_3\)};
  \node[pair,below=of p3] (p4) {\(\mathit{pair}\ c_3\ c_4\)};
  \draw[->] (p0) -- node[right]{\(ss\)：复制并加一} (p1);
  \draw[->] (p1) -- node[right]{\(ss\)：复制并加一} (p2);
  \draw[->] (p2) -- node[right]{\(ss\)：复制并加一} (p3);
  \draw[->] (p3) -- node[right]{\(ss\)：复制并加一} (p4);
  \node[below=3mm of p4] {\(\vdots\)};
\end{tikzpicture}
\caption{前驱函数的“内层循环”}
\label{fig:5-1}
\end{figure}
...
\begin{align*}
 ...
 &\trans
 (\lambda n.\,c_3\,(\mathit{plus}\ n)\,c_0)\,
 (fct\,(\mathit{prd}\ c_3))\\
 &\transmulti
 (\lambda n.\,c_3\,(\mathit{plus}\ n)\,c_0)\,(fct\ c_2')\\
 &\transmulti
 \mathit{times}\ c_3\,(g\ fct\ c_2')\\
 &\transmulti
 \mathit{times}\ c_3\,
   (\mathit{times}\ c_2'\,(g\ fct\ c_1'))\\
 &\transmulti
 \mathit{times}\ c_3\,
   (\mathit{times}\ c_2'\,
     (\mathit{times}\ c_1'\,(g\ fct\ c_0')))\\
 &\transmulti
 \mathit{times}\ c_3\,
   (\mathit{times}\ c_2'\,
     (\mathit{times}\ c_1'\,
 ...
 &\transmulti
 \mathit{times}\ c_3\,
   (\mathit{times}\ c_2'\,(\mathit{times}\ c_1'\ c_1))\\
 &\transmulti c_6'.
\end{align*}
这里 \(c_i'\) 与 \(c_i\) 行为等价。第一处先归约
\(\mathit{times}\ c_3\)，再对参数 \(fct\,(\mathit{prd}\ c_3)\) 求值，
以符合按值调用次序。
\end{minipage}
