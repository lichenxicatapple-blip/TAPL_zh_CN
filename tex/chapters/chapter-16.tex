...
\taplsecref{16.3}{sec:16.3}处理条件式等多分支构造；它们还需要子类型关系
...
不过，在删去传递规则之前，先要重组记录子类型规则。上一章中，宽度、深度和
...
\begin{minipage}{0.98\linewidth}
\small
\taplfigureheader{\(\algturnstile S\subtype T\)}
  {算法子类型关系}
\[
\begin{array}{c@{\qquad}c}
...
处理好子类型关系后，还要以同样方式处理类型关系。唯一不语法导向的类型规则是
...
\]
原书勘误所说的“右侧子推导”，指的正是产生结果类型关系
\(S_{12}\subtype T_{12}\) 的那棵子推导。
...
\begin{minipage}{0.98\linewidth}
\small
\taplfigureheader{\(\Gamma\algturnstile t:T\)}
  {算法类型关系}
\[
\begin{array}{c@{\qquad}c}
