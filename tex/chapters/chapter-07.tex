...
\texttt{code/untyped/} 提供与第 5--7 章形式语义相对应的 Rust 参考实现，
其中包括 de Bruijn
索引、上下文长度一致性检查、移位、替换、按值调用求值、解析与打印；该工程
的语义覆盖和与作者工具的具体语法差异见该目录的 README。}

\section{项与上下文}
\label{sec:7.1}

把定义~\ref{def:6.1.2} 直接转写为数据类型，就得到表示项的抽象语法树：
...
大量乏味的重复；\taplsecref{25.2}{sec:25.2} 会进一步说明。
